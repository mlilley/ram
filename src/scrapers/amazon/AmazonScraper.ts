...
import AmazonSearchResult from "./AmazonSearchResult";
...
export default class AmazonScraper extends BaseScraper {
     ...
     * @param searchText
     */
    public productSearch(searchText: string): Bluebird<AmazonSearchResult[]> {
    ...
    private extractSearchResults(html: string): AmazonSearchResult[] {
        const $ = cheerio.load(html);
        const items: AmazonSearchResult[] = [];
        $("ul#s-results-list-atf").find("li.s-result-item").each((i, domEl) => {
            const el = $(domEl);
            const item = new AmazonSearchResult();
